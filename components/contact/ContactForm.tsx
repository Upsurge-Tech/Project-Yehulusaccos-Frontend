...
const ContactForm = () => {
  ...
  return (
    <div className="w-full rounded-lg p-8">
      <form onSubmit={handleSubmit} action='https://formspree.io/f/mwkggpqv' method="POST" >
      ...
      {status && (
        <div className={`mt-4 md:text-lg text-md text-center ${status === 'success' ? 'text-primary' : 'text-red-600'}`}>
          {status === 'success' ? 'Your response has been recorded successfully!' : 'Failed to send message.'}
        </div>
      )}
    </div>
  );
