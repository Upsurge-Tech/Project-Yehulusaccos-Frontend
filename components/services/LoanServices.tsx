...
const loanServices = [
  { image: "/social.svg", title: "For social services" },
  { image: "/vehicle.svg", title: "For the purchase of car" },
  { image: "/briefcase.svg", title: "For business expansion" },
  { image: "/home.svg", title: "For a purchase of a house" },
];
  ...
  return (
    <div className="flex flex-col w-full gap-y-10 mt-16">
      ...
      <div className="w-full place-items-center grid grid-cols-1 md:grid-cols-2 xl:flex  gap-5 lg:gap-10 mt-2 gap-y-7 xl:gap-x-24 xl:gap-y-16">
        {loanServices.map((ls) => (
          <div key={ls.title} className="flex w-[220px] gap-4 items-center ">
            <Image
              src={ls.image}
              width={30}
              height={30}
              ...
              alt={ls.title}
            />
            <p className="text-sm font-semibold">{ls.title}</p>
          </div>
        ))}
      </div>
    </div>
